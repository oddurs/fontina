...
2. the `UNIFONT_DB` environment variable;
3. the platform data directory:

| Platform | Default |
|---|---|
| Linux and BSDs | `$XDG_DATA_HOME/unifont/index.db`, normally `~/.local/share/unifont/index.db` |
| macOS | `~/Library/Application Support/unifont/index.db` |
| Windows | `%APPDATA%\unifont\index.db` |

These come from the `directories` crate, which follows the XDG Base Directory
specification and the platform conventions. `unifont stats` prints the path in use.

Keep a second index for experiments by setting `UNIFONT_DB` in the shell; nothing
...
```
sqlite3 ~/.local/share/unifont/index.db 'select family, count(*) from faces group by 1 order by 2 desc limit 10'
```

Write to it only through unifont.

## Migrations

The schema version is `PRAGMA user_version`. unifont applies any pending migrations
when it opens the database, in order, inside a transaction. Migrations are
append-only: an applied one is never edited. A migration that needs data unifont
already extracted reads it from the stored metadata JSON, so adding a new indexed
column never needs a rescan. A new version of unifont that extracts *new* metadata
does need `scan --force` to see it in older faces; the changelog says when.

An older unifont refuses to open a newer database rather than guess.

## Moving, backing up, discarding

Copy the single file (with its `-wal` and `-shm` siblings if present, or after
unifont has closed cleanly). Paths inside are absolute, so an index copied to another
...
platform configuration directory (`$XDG_CONFIG_HOME/unifont/config.toml` on Linux),
