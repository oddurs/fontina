...
/// Report what the license permits the user to do. Not a defect in the font file, but
/// the single fact that decides whether it can be studied, changed and passed on, so it
/// is reported at the same volume as one.
fn freedom(c: &mut Ctx) {
    let v = crate::freedom::assess(c.f.license.spdx.as_deref());
    match v.freedom {
        crate::Freedom::Nonfree => c.warn("license/nonfree", format!("nonfree: {}", v.reason)),
        // `license/missing` and `license/unknown` already cover the two ways a font can
        // fail to say; repeating them here would only double the noise.
        crate::Freedom::Unknown | crate::Freedom::Unstated => {}
        crate::Freedom::Free => c.info("license/free", format!("free software: {}", v.reason)),
    }
}

/// `OS/2.fsType` restricts what other programs may do with the font when embedding it.
/// fontina reports the bits and never acts on them: they are the font file's assertion,
/// not a term of the license, and enforcing them would mean restricting the user on
/// their own machine.
fn embedding(c: &mut Ctx) {
    let Some(os2) = &c.f.os2 else { return };
    if !matches!(os2.embedding.level, EmbeddingLevel::Installable) {
        c.info(
            "license/embedding",
            format!(
                "OS/2.fsType is {:#06x} ({:?}); reported, not enforced",
