...
/// `$XDG_DATA_HOME/fonts/unifont`: persistent installs.
pub fn install_dir() -> Result<PathBuf> {
    data_home()
        .map(|d| d.join("fonts").join("unifont"))
        .ok_or(PlatformError::NoUserDir)
}

/// `$XDG_DATA_HOME/fonts/unifont-active`: activations.
pub fn active_dir() -> Result<PathBuf> {
    data_home()
        .map(|d| d.join("fonts").join("unifont-active"))
        .ok_or(PlatformError::NoUserDir)
}

/// `$XDG_CONFIG_HOME/fontconfig/conf.d/50-unifont.conf`.
pub fn fontconfig_snippet() -> Option<PathBuf> {
    config_home().map(|c| c.join("fontconfig").join("conf.d").join("50-unifont.conf"))
}

const SNIPPET: &str = r#"<?xml version="1.0"?>
<!DOCTYPE fontconfig SYSTEM "fonts.dtd">
<!-- Written by unifont. Fonts activated with `unifont activate` are linked here. -->
<fontconfig>
  <dir prefix="xdg">fonts/unifont-active</dir>
  <dir prefix="xdg">fonts/unifont</dir>
</fontconfig>
"#;
...
impl FontActivator for Fontconfig {
    ...
    fn activate(&self, file: &Path, _scope: Scope) -> Result<()> {
        ...
        // which is which and `unifont restore` (or a login agent) re-links session ones.
        let dir = active_dir()?;
        link_into(&dir, file)?;
        ensure_snippet()
    }
...
mod tests {
    ...
    fn sandbox(name: &str) -> (PathBuf, PathBuf) {
        let root =
            std::env::temp_dir().join(format!("unifont-linux-{name}-{}", std::process::id()));
    ...
    fn install_links_into_xdg_and_writes_the_snippet() {
        ...
        assert_eq!(link, data.join("fonts/unifont/Amiri-Regular.ttf"));
        assert_eq!(
            std::fs::read_link(&link).unwrap(),
            std::fs::canonicalize(fixture()).unwrap()
        );
        assert_eq!(a.install(&fixture()).unwrap(), link, "idempotent");
        let snippet = config.join("fontconfig/conf.d/50-unifont.conf");
        assert!(snippet.exists());
        assert!(
            std::fs::read_to_string(&snippet)
                .unwrap()
                .contains("unifont-active")
        );
    ...
    fn activate_and_deactivate_by_target() {
        ...
        let active = data.join("fonts/unifont-active");
