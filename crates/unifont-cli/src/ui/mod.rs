//! `unifont ui`: a keyboard-first browser over the index. Facets on the left, families
...
use unifont_core::index::FacetCount;
use unifont_core::{ActivationState, FaceFilter, FaceMetadata, FaceSummary, Facets, Family, Index};
...
impl App {
    fn new(index: Index) -> Result<Self> {
        ...
        if app.families.is_empty() && app.selected.is_empty() && app.query.is_empty() {
            app.status =
                "index is empty: run `unifont scan <dir>` or `unifont scan --system`".into();
        }
        Ok(app)
    }
                ...
                match kind {
                    ...
                    InputKind::Tag => {
                        if !value.is_empty() {
                            ...
                            self.status = format!(
                                "tagged {n} face(s) with {value:?}   (unifont tag add {} <targets>)",
                                shell_quote(&value)
                            );
                            self.reload()?;
                        }
                    ...
                    InputKind::Collection => {
                        if !value.is_empty() {
                            ...
                            self.status = format!(
                                "added {n} face(s) to {value:?}   (unifont collection add {} <targets>)",
                                shell_quote(&value)
                            );
                            self.reload()?;
                        }
    ...
    fn activate(&mut self, state: ActivationState) -> Result<()> {
        ...
        if !conflicts.is_empty() {
            let c = &conflicts[0];
            self.status = format!(
                "{} conflict(s): {} {} ({}). Use `unifont activate --replace` to override.",
                conflicts.len(),
                c.face.family,
                c.face.subfamily,
                c.reason
        ...
        self.status = format!("{verb}: {n} face(s)   (unifont {verb} <targets>)");
        self.reload()
    }
    ...
    fn rescan(&mut self) -> Result<()> {
        ...
        let report = unifont_core::scan::scan(
            &mut self.index,
            &roots,
            &unifont_core::ScanOptions {
                prune: true,
                ..Default::default()
            },
        )?;
        self.status = format!(
            "rescanned {} source(s): {} parsed, {} unchanged, {} removed, {} failed   (unifont scan --prune)",
            roots.len(),
            report.parsed,
            report.unchanged,
            report.removed,
 ...
 Index       R rescans every source (unifont scan --prune)
 Quit        q
...
fn facet_value_label(facet: Facet, value: &str) -> String {
    match facet {
        Facet::Weight => format!(
            "{value} {}",
            unifont_core::index::weight_name(value.parse().unwrap_or(400))
        ),
        Facet::Width => format!(
            "{value}% {}",
            unifont_core::index::width_name(value.parse().unwrap_or(100.0))
        ),
